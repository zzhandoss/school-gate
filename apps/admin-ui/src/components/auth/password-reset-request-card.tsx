      ...
      <CardContent className="grid p-0 md:grid-cols-2">
        <form className="space-y-4 p-6 md:p-8" onSubmit={form.handleSubmit(onSubmit)}>
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Enter your admin email. If account exists, reset instructions will
              be issued.
            </p>
          </div>
          ...
          <Button type="submit" className="w-full" disabled={mutation.isPending}>
            {mutation.isPending ? 'Sending reset request…' : 'Send reset request'}
          </Button>
          ...
          {mutation.isSuccess ? (
            ...
            >
              ...
              <AlertDescription>
                If the account exists, a reset token is now active.
                {issuedToken ? (
                  <>
                    {' '}
                    Dev token:
        ...
        <div className="relative hidden overflow-hidden bg-gradient-to-br from-indigo-100 via-white to-teal-100 p-8 md:flex md:flex-col md:justify-between">
          ...
          <div className="relative z-10 space-y-3">
            <h2 className="text-2xl font-semibold text-slate-900">Recover admin access</h2>
            <p className="max-w-xs text-sm text-slate-700">
              Reset links are one-time and expire automatically.
            </p>
            <div className="inline-flex items-center gap-2 text-xs text-slate-600">
              <MailCheck className="h-4 w-4 text-emerald-600" />
              Use only trusted admin email
            </div>
          </div>
        </div>
      </CardContent>
